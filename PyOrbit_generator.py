<<<<<<< Updated upstream:pyORBIT/pyOR_lattice_generator.py
...
def generator(dir='.', file='simuINwork', ext='yml', EzFile=None):
    input   = '{}/{}.{}'.format(dir,file,ext)
    output  = '{}/{}.{}'.format('.','PyOR_lattice','xml')
    ...
    for section in sections:
        ...
        sec_da.setValue('length',section['length'])
        sec_da.setValue('name',sec)
        DEBUG_ON('sec_da: {}'.format(sec_da.makeXmlText()))
        cavs_da  = sec_da.createChild('Cavities')
        gap_cnt  = 0
        quad_cnt = 0
        ...
        for node in section['seq']:
            ...
            if isinstance(node,(ELM.GAP, ELM.RFC)):
                ...
                if isinstance(node,(ELM.QF,ELM.QD)):
                    ...
                    pass
            DEBUG_ON('accelm_da: {}'.format(accelm_da.makeXmlText()))
...
>>>>>>> Stashed changes:pyOR_lattice_generator.py
