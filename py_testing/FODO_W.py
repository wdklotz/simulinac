...
def test2():
    ...
    print('probe: mfodo-mfodo2 must be zero matrix')
    zero = mfodo-mfodo2
    print(f'{abs(zero[0][0]):.5f}  {abs(zero[0][1]):.5f}')
    print(f'{abs(zero[1][0]):.5f}  {abs(zero[1][1]):.5f}')
    ...
    for node in lattice:
        ...
        mfodo3 = mfodo4
    print('probe: mfodo3-mfodo5 must be zero matrix')
    zero = mfodo3-mfodo5
    print(f'{abs(zero[0][0]):.5f}  {abs(zero[0][1]):.5f}')
    print(f'{abs(zero[1][0]):.5f}  {abs(zero[1][1]):.5f}')
    ...
    beta = 1.          # [m]
    alfa = 0.
    epsi = 1.e-6      # emiitance [m*rad]
    
    twx = Twiss(beta,alfa,epsi)
    track_point = np.array(twx.y1())    # C track
    points = [track_point]
    for node in lattice:
        tracked = node.dot(track_point)
        points.append(tracked)
        track_point = tracked

    x  = [v[0] for v in points]
    xp = [v[1] for v in points]
    plt.figure('x & xp')
    plt.plot(pos,x, label='x')
    plt.plot(pos,xp, label='xp')
