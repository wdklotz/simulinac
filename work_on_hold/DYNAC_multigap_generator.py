...
def field_repeat(arg):
    """ E.Tanke's Vorschlag: ...das SF-Feld mehrfach zu wiederholen.   Author: Fazel.T  """
    n                     = arg['n']
    cavlen                = arg['cavlen'] *.5  # (m) 1/2 cavity length
    cav_cnt               = arg['cav_cant_fir']
    sfdata                = arg['sfdata'] 
    sfdtable              = sfdata._Ez0_tab_scaled   # normed to EzPeak
    table_file_name_add   = f'dynacEzTab_{cav_cnt}' 
    tmp                   = []
    ...
    sfdtable = tmp
    z0 = sfdtable[0].z
    field = [[(p.z - z0)*1.e-2+(n-1)*cavlen*2*1.e-2, p.Ez*1.e6] for p in sfdtable]  # z[m], EzPeak[V/m]

   # # delet (0,0) from end of last field
    with open(output_base_dir + table_file_name_add, "r") as field_table:
        tn      = field_table.readlines()
        n       = len(tn)
        del tn[n-1]
        del tn[n-2]
    with open(output_base_dir + table_file_name_add, "w") as f:
        f.writelines(tn)

   #write new field for next cell on the  data file
    with open(output_base_dir + table_file_name_add, 'a') as field_table:
...
def call_ALCELI(arg):
    ...
    node_cnt, cav_cnt, quad_cnt, cavnum_cnt = 0,0,0,0
    ...
    for node in lattice.seq:
        if isinstance(node, (ELM.QF, ELM.QD)):
            ...
            if sfdata == None:
                ...
                if lm != diff_cav_nod:
                    ...
                    field_add =dict(
                        ...
                        sfdata             = sfdata,
                        cavlen             = cavlen,
                        cav_cant_fir       = cav_cant_first,
                    )
                    field_repeat(field_add)
            else:
                n = lk           = 1
                active_field_map = sfdata
                cav_cant_first   = cav_cnt
            ...
            if lm != diff_cav_nod:
                ...
                cavnum_params = dict(
                    file      = file,
                    number    = cavnum_cnt,   # dummy DYNAC variable
                    dphase    = dphase,
                    intrvl    = 6,
                    ffield    = 0.
                )
    ...
    print(f"nodes {node_cnt}, quadrupoles {quad_cnt}, rf-gaps {cav_cnt}, CAVNUMs {cavnum_cnt}")
def call_FINISH(arg):
    file = arg['file']
    file.write("STOP\n")
    file.close()
...
if __name__ == '__main__':
    ...
    # Parameter: import all the parameters and convert them to the correct units 
    # apart from the accelerating and transport elements which are imported later.
